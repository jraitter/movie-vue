...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async searchMovie({ commit, dispatch }, searchString) {
      try {
        let res = await _api.get(`search/movie?api_key=606e6aee588b47993fffe6d9530d07a6&page=1&include_adult=false&query=${searchString}`);
    ...
    async getPopularMovies({ commit, dispatch }) {
      try {
        let res = await _api.get("discover/movie?api_key=606e6aee588b47993fffe6d9530d07a6&page=1&include_adult=false&sort_by=popularity.desc");
        commit("setMovies", res.data.results);
      } catch (error) {
        console.error(error);
      }
